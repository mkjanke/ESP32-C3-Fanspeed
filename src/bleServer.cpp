...
void bleInterface::begin() {
  ...
  temperatureA_BLEC =
      pService->createCharacteristic(TEMPERATURE_UUID, NIMBLE_PROPERTY::READ);
  temperatureB_BLEC =
      pService->createCharacteristic(HUMIDITY_UUID, NIMBLE_PROPERTY::READ);
  uptimeBLEC =
      pService->createCharacteristic(UPTIME_UUID, NIMBLE_PROPERTY::READ);
  fanspeedBLEC =
      pService->createCharacteristic(FANSPEED_UUID, NIMBLE_PROPERTY::READ);
  ...
  fanOverrideBLEC = pService->createCharacteristic(
      FANOVERRIDE_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE);
  fanRelayBLEC = pService->createCharacteristic(
      RELAY_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE);

  fanStartTempBLEC->setCallbacks(new intCallbacks(
      &fan.startTemp, sizeof(uint8_t)));  // Fan Start Temperature
  fanMaxTempBLEC->setCallbacks(
      new intCallbacks(&fan.maxTemp, sizeof(uint8_t)));  // Fan Max Temperature
