...
class PWMFan {
 public:
  ...
  uint8_t maxTemp =
      FAN_MAX_TEMP;  // Temp at which fan is at max speed (100% pulse width)
  ...
  void setFanSpeed(int dhtTempA, int dhtTempB) {
    ...
    //              'B' is lower sensor, below coils, presumed to be ambient
    //              temperature
    //
    // Fan speed will be proportional to difference between A and B, bounded by
    // HIGH and LOW limits
    ...
    if (override)
      ...
      startTemp =
          (startTemp < FAN_TEMP_LOW_LIMIT) ? FAN_TEMP_LOW_LIMIT : startTemp;
